...
function IsAuthorized(xPlayer, doorID, locked, usedLockpick)
	...
	grade[1] = tonumber(xPlayer.job.grade)
	if xPlayer.job2 then
		jobName[2] = xPlayer.job2.name
		grade[2] = tonumber(xPlayer.job2.grade)
	end
	local canOpen = false
	...
	if not canOpen and doorID.authorizedJobs then
		for job,rank in pairs(doorID.authorizedJobs) do
			if (job == jobName[1] and tonumber(rank) <= grade[1]) or (jobName[2] and job == jobName[2] and tonumber(rank) <= grade[2]) then
				canOpen = true
