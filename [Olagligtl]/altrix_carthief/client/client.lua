                    ...
                    if DoesEntityExist(ped) then
                        while #(GetEntityCoords(Self) - GetEntityCoords(ped)) <= 5.0 and not IsEntityPlayingAnim(ped, Config['Settings']['Animation']['Dict'], Config['Settings']['Animation']['Anim'], 3) do
                            ...
                            if IsControlJustReleased(0, 51) then
                                ESX.TriggerServerCallback('altrix_carthief:canStart', function(can)
                                    CreateThread(function()
                                        if can and not InMission then
                                                ...
                                                if not HasStolenCar then
                                                    ...
                                                    if #(GetEntityCoords(Self) - position.xyz) <= 75.0 then
                                                        if not DoesEntityExist(vehicle) then
                                                            ...
                                                            SetVehicleNumberPlateText(vehicle, 'NRPCAR')
                                                        else
                                                            ...
                                                            elseif IsPedInVehicle(Self, vehicle, false) then
                                                                ...
                                                                if IsControlJustReleased(0, 47) then
                                                                    ...
                                                                    if hotwire == 1000 then
                                                                        ...
                                                                        RemoveBlip(blip)
                                                                        ClearPedTasks(Self)
                                                                        TriggerServerEvent('altrix_carthief:setNetId', ObjToNet(vehicle))
                                                                    end
                                                                end
                                                            end
                                                        end
                                                    ...
                                                    if transmitter <= GetGameTimer() then
                                                        if not killedblip then
                                                            killedblip = true
                                                            TriggerServerEvent('altrix_carthief:deleteBlip')
                                                        end
                                            ...
                                            if not Stopped then
                                                RemoveBlip(blip)
                                                TriggerServerEvent('altrix_carthief:reward', GetVehicleBodyHealth(vehicle), GetVehicleEngineHealth(vehicle))
                                                DeleteEntity(vehicle)
                                            else
                                                RemoveBlip(blip)
                                                TriggerServerEvent('altrix_carthief:end')
                                            end
...
RegisterNetEvent('altrix_carthief:setBlip')
AddEventHandler('altrix_carthief:setBlip', function(netid)
...
RegisterNetEvent('altrix_carthief:removeBlip')
AddEventHandler('altrix_carthief:removeBlip', function()
    if DoesBlipExist(policeblip) then
        RemoveBlip(policeblip)
    end
end)