...
AddEventHandler('es_db:createUser', function(identifier, license, cash, bank, callback)
	...
	MySQL.Async.execute('INSERT INTO users (`identifier`, `group`, `permission_level`, `license`) VALUES (@identifier, @group, @permission_level, @license);', {identifier = user.identifier, permission_level = user.permission_level, group = user.group, license = user.license}, function(e)
		callback()
	end)
end)
