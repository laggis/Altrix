...
CreateThread(function()
    ...
    if Stripssss.Framework == "esx" then
        while not ESX do
            Wait(500)
    ...
    elseif Stripssss.Framework == "qb" then
        while not QBCore do
            Wait(500)
    ...
    if Stripssss.RequireJobPlace then
        if Stripssss.Framework == "esx" or Stripssss.Framework == "qb" then
            local eventName = Stripssss.Framework == "esx" and "esx:setJob" or "QBCore:Client:OnJobUpdate"
            RegisterNetEvent(eventName)
            AddEventHandler(eventName, function(jobData)
                local jobName = jobData.name
                isPolice = false
                for _, job in pairs(Stripssss.FrameworkFeatures.PoliceJobs) do
                    if job == jobName then
                        isPolice = true
                        break
                    end
            ...
            local jobName = Stripssss.Framework == "esx" and ESX.GetPlayerData().job.name or QBCore.Functions.GetPlayerData().job.name
            for _, job in pairs(Stripssss.FrameworkFeatures.PoliceJobs) do
                if job == jobName then
                    isPolice = true
                    break
    ...
    CreateThread(function()
        while true do
            Wait(500)
            if (isPolice or not Stripssss.RequireJobRemove) and IsPedOnFoot(PlayerPedId()) then
                while (isPolice or not Stripssss.RequireJobRemove) and DoesEntityExist(closestStinger) and closestStingerDistance <= 4.0 and IsPedOnFoot(PlayerPedId()) do
                    Wait(0)
