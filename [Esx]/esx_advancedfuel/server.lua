...
AddEventHandler("essence:buy", function(amount, index,e)
	...
	local toPay = round(amount * 4,2)
	
	if(toPay > xPlayer.getMoney()) then
		TriggerClientEvent('notification', _source, "Du har inte ~r~tillräckligt~s~ med pengar.")
	else
...
AddEventHandler("essence:buyCan", function()
	...
	if(toPay > xPlayer.getMoney()) then
		TriggerClientEvent('notification', _source, "Du har inte tillräckligt med pengar.")
	else
		xPlayer.removeMoney(toPay)
...
function renderPrice()
    for i=0,34 do
		StationsPrice[i] = math.random(14, 16)
    end
end
