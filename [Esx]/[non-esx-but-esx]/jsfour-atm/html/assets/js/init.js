$(document).ready(function(){
  ...
  var cash = 0;
  var bank = 0;
  var creditCardCode = 1234;
  var currentCardOwner = "Peter Pan";
  ...
  window.addEventListener('message', function(event) {
      if (event.data.action == 'open') {
        ...
        currentCardOwner = event.data.owner;
        creditCardCode = event.data.code
        ...
        if (cash > 0) {
          if (amount == 2000) {
            ...
            if (inside) {
              ...
              $.post('http://jsfour-atm/insert', JSON.stringify({money : cash, owner : currentCardOwner}));
            ...
            if (inside) {
              ...
              $.post('http://jsfour-atm/insert', JSON.stringify({money : amount, owner : currentCardOwner}));
          ...
          if (bank > 0 && bank > amount) {
            ...
            setTimeout(function() {
              ...
              setTimeout(function() {
                ...
                if (inside) {
                  $('#header p').text( parseInt($('#header p').text()) - parseInt(amount));
                  $.post('http://jsfour-atm/take', JSON.stringify({money : amount, owner : currentCardOwner}));
                  cash = parseInt(cash) + parseInt(amount);
                }
              }, 3800);
            }, 5600);
