...
function StartDriveTest(type)
  ...
  ESX.Game.SpawnVehicle(Config.VehicleModels[type], Config.Zones.VehicleSpawnPoint.Pos, 317.0, function(vehicle)
    ...
    props.plate = 'altrixTRAF'

    ESX.Game.SetVehicleProperties(vehicle, props)

    exports["altrix_keysystem"]:GiveTempKey(props.plate)
...
function OpenDMVSchoolMenu()
  ...
  ESX.TriggerServerCallback("rdrp_dmv:fetchLicenses", function(licenseArray)
    licenses = licenseArray
  end, GetPlayerServerId(PlayerId()))

  ESX.ShowNotification("Pratar...", "", 1000)
  ...
  if not dmvLicense then
    table.insert(elements, {label = 'Teoriprov - ' ..Config.Prices["dmv"] .. ' SEK ', value = 'theory_test', type = 'drive'})
  else
    if not driveLicense then
      table.insert(elements, {label = 'B-Körkort - ' ..Config.Prices["drive"] .. ' SEK ', value = 'drive_test', type = 'drive'})
    end

    if not bikeLicense then
      table.insert(elements, {label = 'A1-Körkort - ' ..Config.Prices["drive_bike"] .. ' SEK ', value = 'drive_test', type = 'drive_bike'})
    end

    if not truckLicense then
      table.insert(elements, {label = 'C-Körkort - ' ..Config.Prices["drive_truck"] .. ' SEK ', value = 'drive_test', type = 'drive_truck'})
    end
  end
