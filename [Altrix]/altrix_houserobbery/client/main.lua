...
AddEventHandler("esx:playerLoaded", function(response)
	...
	ESX.TriggerServerCallback("rdrp_houserobbery:retrieveBreakInformation", function(house)
		if house ~= "none" then
			Config.HousesToRob[house]["InProgress"] = true

			if ESX.PlayerData["job"]["name"] == "police" then
				TriggerEvent("rdrp_houserobbery:alertPolice", house)
			end
		end
	end)
end)
...
RegisterNetEvent("rdrp_houserobbery:addSkill")
AddEventHandler("rdrp_houserobbery:addSkill", function(skillLevel)
	exports["altrix_skills"]:AddSkillLevel("lockpick", skillLevel)
end)

RegisterNetEvent("rdrp_houserobbery:openHouse")
AddEventHandler("rdrp_houserobbery:openHouse", function(response)
	Config.HousesToRob[response]["InProgress"] = true
end)

RegisterNetEvent("rdrp_houserobbery:resetHouse")
AddEventHandler("rdrp_houserobbery:resetHouse", function(response)
...
RegisterNetEvent("rdrp_houserobbery:spotSearched")
AddEventHandler("rdrp_houserobbery:spotSearched", function(house, response)
	Config.HousesToRob[house]["RobSpots"][response]["stolen"] = true
end)

RegisterNetEvent("rdrp_houserobbery:alertPolice")
AddEventHandler("rdrp_houserobbery:alertPolice", function(house)
...
function LockHouse(houseN)
	TriggerServerEvent("rdrp_houserobbery:lockHouse", houseN)
end

function TryBreakIn(houseN)
	local haslockpick = exports["qalle-base"]:GetInventoryItem("lockpick")
	...
	if Config.HousesToRob[houseN]["lockpickLevel"] ~= nil then
		local lockpickLevel = exports["altrix_skills"]:GetSkillLevel("lockpick")

		if lockpickLevel < Config.HousesToRob[houseN]["lockpickLevel"] then
			ESX.ShowNotification("Du har ej lärt dig tillräckligt ännu. Du har ~g~" .. lockpickLevel .. "% / " .. Config.HousesToRob[houseN]["lockpickLevel"] .. "%")
			return
		end
	end
	...
	while math.ceil(Percent) <= 100 do
		...
		if DoesEntityExist(ped) and pedDst <= 15.0 and not seen then
			...
			TriggerServerEvent("esx_phone:send", "police", "Jag såg någon bryta sig in i en fastighet! Jag fick en bild på honom!", true, { x = pedPos.x, y = pedPos.y, z = pedPos.z })
	...
	if IsEntityPlayingAnim(PlayerPedId(), "mini@safe_cracking", "idle_base", 3) then
		TriggerServerEvent("rdrp_houserobbery:houseOpen", houseN)
		local lowEndHouse = Config.HousesToRob[houseN]["LowEnd"] ~= nil
		if not lowEndHouse then
		TriggerServerEvent("esx_phone:send", "police", "Verisure-larm utlöst. Bild skickad!", true, { x = Config.HousesToRob[houseN]["Enter"].x, y = Config.HousesToRob[houseN]["Enter"].y, z = Config.HousesToRob[houseN]["Enter"].z })
...
function EnterHouse(houseName)
	...
	StartSearching = function(robSpot)
		...
		if Percent >= 100 then
			ClearPedTasks(PlayerPedId())
			TriggerServerEvent("rdrp_houserobbery:retrieveRandomItem", robSpot, houseName)
...
OpenStolenItemsMenu = function(house)
	...
	ESX.TriggerServerCallback("rdrp_houserobbery:fetchStolenItems", function(found, data)
		if found then
