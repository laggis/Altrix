...
RegisterCommand('luftkudde', function(source, args, rawCommand)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
...
function createAirbags(vehicle, temp)
    ...
    local driat = GetEntityBoneIndexByName(vehicle, "seat_dside_f")
    ...
    AttachEntityToEntity(airbag1, vehicle, driat, 0.0, 0.30, 0.40, 90.0, 0.0, 0.0, true, true, false, false, 2, true)
